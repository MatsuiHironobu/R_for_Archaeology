...
```{r cross-hist, fig.cap='クロス集計表から再構成したヒストグラム'}
muraki2004_long %>%
  ggplot() +
...
```{r datasaurus-plot, echo=FALSE, fig.cap="可視化の重要性"}
# 可視化
datasaurus_dozen %>%
...
```{r theme-plot, echo=FALSE, fig.show="hold", out.width="48%", fig.cap="`theme_*()`の表示例"}
# デフォルト
...
```{r theme-plot2, fig.cap="`theme()`の設定例"}
...
```{r my-theme, fig.cap="`theme()`で好みの見た目に設定できる"}
...
```{r hist, fig.cap='geom_histogramのデフォルト表示', message=FALSE, warning=FALSE}
hist <- fish %>%
  ggplot() +
...
```{r hist2, fig.cap='`position = "stack"`での描写'}
hist_2 <- fish %>%
...
```{r hist3, fig.cap='`facet_weap()`の描写'}
hist_3 <- fish %>%
  dplyr::filter(Fish == "クロダイ属"  &
                  Direction == "左" & 
                  Period %in% c("1期", "2期", "3期") &
...
```{r hist-build, message=FALSE, warning=FALSE}
...
```{r bar, fig.cap='geom_barのデフォルト表示'}
bar <- fish %>%
...
```{r bar3, fig.cap='構成する種類をまとめた帯グラフ', message=FALSE}
#### ラベルを作成する ####
# 魚種の出土数トップ9を抽出
top5 <- fish %>%
  dplyr::filter(Period %in% c("1期", "2期", "3期")) %>%
...
```{r bar4, fig.cap='geom_bar_patternでの論文風の帯グラフ'}
...
```{r sina, fig.show="hold", out.width="48%", fig.cap="ジッタープロット（左）とシナプロット（右）"}
#### ジッタープロット ####
jitter <- fish %>%
  dplyr::filter(Fish == "クロダイ属"  &
                  Period %in% c("1期", "2期", "3期") &
...
```{r box-violin-sina, fig.cap="箱ひげ図と重ねて表示"}
